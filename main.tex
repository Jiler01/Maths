...
Si on en rencontre une, l'usage est de manipuler l'expression de sorte à ne plus avoir ce problème, ou bien d'appliquer la règle de L'Hôpital selon laquelle $$\lim \limits_{x \Rightarrow e} \frac{f(x)}{g(x)} = \lim \limits_{x \Rightarrow e} \frac{f'(x)}{g'(x)}$$ ssi $f'(x)$ et $g'(x)$ existent et $\lim \limits_{x \Rightarrow e} \frac{f(x)}{g(x)} = \frac{0}{0}$ ou $\frac{\pm\infty}{\pm\infty}$.
