...
\begin{itemize}
	...
	\item $\exists$ signifie "il existe", $\exists!$ siginifie "il existe un unique"
\end{itemize}
...
La fonction valeur absolue est définie par $f(x) = |x| $. Elle est paire sur sur $\mathbb{R}$, décroissante sur $\mathbb{R^-}$ et croissante sur sur $\mathbb{R^+}$.\\
...
\paragraph{Cosinus}
$\cos$ est une fonction paire, variant entre $-1$ et $1$, et de période $2\pi$, tel que $\cos(x + 2k\pi) = \cos(x)$, $\forall k \in \mathbb{N}$.\\
\break
\paragraph{Sinus}
$\sin$ est une fonction impaire, variant entre $-1$ et $1$, et de période $2\pi$, tel que $\sin(x + 2k\pi) = \sin(x)$, $\forall k \in \mathbb{N}$.
...
On note la norme $AB$ du vecteur $\Vec{AB}$ : $||\Vec{AB}||$.\\
Dans un repère orthonormé (euclidien), $||\Vec{AB}||$ peut se calculer avec la formule de la distance euclidienne, comme suit: $\sqrt{(x_B-x_A)^2+ (y_B-y_A)^2+...}$.
...
Si il existe des réels $\alpha,\beta\in\mathbb{R}$ $\Vec{w}=\alpha\Vec{u}+\beta\Vec{v}$ alors $\Vec{u},\Vec{v},\Vec{w}$ sont coplanaires.\\
...
\pagebreak\pagebreak\part{Arithmétique}
\setcounter{section}{0}
\renewcommand*{\theHsection}{chX.\the\value{section}}
